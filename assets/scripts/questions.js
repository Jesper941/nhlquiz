/*My array of questions and answers for the quiz*/

const questions = [

    {
        question: "What year was the NHL founded?",
        answers: [
            { text: "1892", correct: false },
            { text: "1947", correct: false },
            { text: "1920", correct: false },
            { text: "1917", correct: true },]
    },
    {
        question: "Who holds the record for the most goals scored in a single NHL season?",
        answers: [
            { text: "Wayne Gretzky", correct: false },
            { text: "Mario Lemieux", correct: false },
            { text: "Bobby Hull", correct: false },
            { text: "Wayne Gretzky", correct: true },
        ]
    },
    {
        question: "What is the oldest NHL team still in existence?",
        answers: [
            { text: "Montreal Canadiens", correct: true },
            { text: "Toronto Maple Leafs", correct: false },
            { text: "Boston Bruins", correct: false },
            { text: "Chicago Blackhawks", correct: false },
        ]
    },
    {
        question: "Which NHL player is known as 'The Great One'?",
        answers: [
            { text: "Bobby Orr", correct: false },
            { text: "Wayne Gretzky", correct: true },
            { text: "Gordie Howe", correct: false },
            { text: "Sidney Crosby", correct: false },
        ]
    },
    {
        question: "In which year did the NHL introduce the 2-line pass rule to open up the game?",
        answers: [
            { text: "1990", correct: false },
            { text: "2000", correct: false },
            { text: "2005", correct: false },
            { text: "2005", correct: true },
        ]
    },
    {
        question: "Which team won the first-ever Stanley Cup in NHL history in 1893?",
        answers: [
            { text: "Montreal Canadiens", correct: false },
            { text: "Ottawa Senators", correct: true },
            { text: "Toronto Maple Leafs", correct: false },
            { text: "Boston Bruins", correct: false },
        ]
    },
    {
        question: "How many players are on the ice for each team during a regular NHL game?",
        answers: [
            { text: "5", correct: false },
            { text: "6", correct: true },
            { text: "7", correct: false },
            { text: "4", correct: false },
        ]
    },
    {
        question: "Which NHL team was known as the 'Broad Street Bullies' due to their physical style of play in the 1970s?",
        answers: [
            { text: "New York Rangers", correct: false },
            { text: "Philadelphia Flyers", correct: true },
            { text: "Chicago Blackhawks", correct: false },
            { text: "Los Angeles Kings", correct: false },
        ]
    },
    {
        question: "Who is the all-time leader in career points scored in the NHL?",
        answers: [
            { text: "Mario Lemieux", correct: false },
            { text: "Gordie Howe", correct: true },
            { text: "Wayne Gretzky", correct: false },
            { text: "Jaromir Jagr", correct: false },
        ]
    },
    {
        question: "Which NHL team has won the most Stanley Cup championships?",
        answers: [
            { text: "Montreal Canadiens", correct: true },
            { text: "Toronto Maple Leafs", correct: false },
            { text: "Boston Bruins", correct: false },
            { text: "Detroit Red Wings", correct: false },
        ]
    },
    {
        question: "In what year did the NHL expand beyond the 'Original Six' teams?",
        answers: [
            { text: "1967", correct: true },
            { text: "1950", correct: false },
            { text: "1975", correct: false },
            { text: "1980", correct: false },
        ]
    },
    {
        question: "Which player holds the record for the most career assists in the NHL?",
        answers: [
            { text: "Bobby Orr", correct: false },
            { text: "Mario Lemieux", correct: false },
            { text: "Wayne Gretzky", correct: true },
            { text: "Joe Sakic", correct: false },
        ]
    },
    {
        question: "Which NHL team was originally known as the Atlanta Thrashers before relocating?",
        answers: [
            { text: "Arizona Coyotes", correct: false },
            { text: "Columbus Blue Jackets", correct: false },
            { text: "Winnipeg Jets", correct: true },
            { text: "Florida Panthers", correct: false },
        ]
    },
    {
        question: "What is the maximum number of games a team can play in a single NHL season?",
        answers: [
            { text: "82", correct: true },
            { text: "84", correct: false },
            { text: "86", correct: false },
            { text: "88", correct: false },
        ]
    },
    {
        question: "Which player was famously known as 'Mr. Hockey' during his NHL career?",
        answers: [
            { text: "Bobby Orr", correct: false },
            { text: "Wayne Gretzky", correct: false },
            { text: "Gordie Howe", correct: true },
            { text: "Maurice Richard", correct: false },
        ]
    },
    {
        question: "What was the first Canadian team to win the Stanley Cup after the NHL's 1967 expansion?",
        answers: [
            { text: "Montreal Canadiens", correct: false },
            { text: "Toronto Maple Leafs", correct: false },
            { text: "Calgary Flames", correct: false },
            { text: "Edmonton Oilers", correct: true },
        ]
    },
    {
        question: "Which NHL team plays their home games at Madison Square Garden?",
        answers: [
            { text: "New York Rangers", correct: true },
            { text: "Boston Bruins", correct: false },
            { text: "Chicago Blackhawks", correct: false },
            { text: "Los Angeles Kings", correct: false },
        ]
    },
    {
        question: "Who was the first goaltender in NHL history to score a goal in a game?",
        answers: [
            { text: "Martin Brodeur", correct: true },
            { text: "Patrick Roy", correct: false },
            { text: "Dominik Hasek", correct: false },
            { text: "Terry Sawchuk", correct: false },
        ]
    },
    {
        question: "Which NHL team is known for their 'Flying V' jersey design from the 1990s?",
        answers: [
            { text: "Anaheim Ducks", correct: true },
            { text: "San Jose Sharks", correct: false },
            { text: "Florida Panthers", correct: false },
            { text: "Colorado Avalanche", correct: false },
        ]
    },
    {
        question: "What is the term for a player scoring three goals in a single NHL game?",
        answers: [
            { text: "Hat trick", correct: true },
            { text: "Power play", correct: false },
            { text: "Faceoff", correct: false },
            { text: "Breakaway", correct: false },
        ]
    },
    {
        question: "Who was the first Russian-born player to be inducted into the Hockey Hall of Fame?",
        answers: [
            { text: "Sergei Fedorov", correct: false },
            { text: "Pavel Bure", correct: false },
            { text: "Igor Larionov", correct: true },
            { text: "Alex Ovechkin", correct: false },
        ]
    }
];